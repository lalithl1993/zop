...
$de_sub=$_GET['sub'];
$sub_id=base64_decode($de_sub);

$ban_qry=mysql_query("select main_category from sub_category where id='$sub_id'");
...
if(!isset($_SESSION['from_date']) or !isset($_SESSION['to_date']) )
	{
		...
		$_SESSION['to_date']=date("d-m-Y", strtotime("+1 month"));
		$_SESSION['db_t_date']=date("Y-m-d", strtotime("+1 month"));
...
if(isset($_POST['book_now']))
{
	...
	$qry=mysql_query("select monthly from product_details where sub_category='$sub_id' and vendor_id='$new_ven_id'");
	$res=mysql_fetch_array($qry);
	$day_price=$res['monthly'];
	$total_price=$day_price;
	
	//$ins=mysql_query
	//("INSERT INTO booking_details(booking_id,user_id,booked_date,booked_time,name,email,phone,address,vendor_id,sub_category,from_date,to_date,total_price,status) 
	//VALUES ('$booking_id', '$user_id', '$today_date', '$current_time', '$name', '$email', '$phone', '$address', '$new_ven_id', '$sub_category', '$db_from_date', '$db_to_date', '$total_price', 'Success')");			
	
	$ins=mysql_query
	("INSERT INTO booking_details(booking_id,user_id,booked_date,booked_time,name,email,phone,address,vendor_id,sub_category,total_price,status) 
	VALUES ('$booking_id', '$user_id', '$today_date', '$current_time', '$name', '$email', '$phone', '$address', '$new_ven_id', '$sub_category', '$total_price', 'Success')");			
		...
		/*----------SMS START For Admin----------------------*/
								...
								$message  = "New Request for Monthly\n";
								$message .= "Name   : ".$name."\n";
								$message .= "Email  : ".$email."\n";
								$message .= "Phone  : ".$phone."\n";
								...
								$msg_body_final=urlencode($msg);
								 ...
								 $URL="http://roundsms.com/api/sendhttp.php?authkey=OTFmM2Y1ZDJkNDd&mobiles=".$mobl1."&message=".$msg_body_final."&sender=ZOPRNT&type=1&route=3";
		                         $ch = curl_init();
								...
								curl_close($ch);
								 ...
								 $URL="http://roundsms.com/api/sendhttp.php?authkey=OTFmM2Y1ZDJkNDd&mobiles=".$mobls."&message=".$msg_body_final."&sender=ZOPRNT&type=1&route=3";
		                        $ch1 = curl_init();
								...
								$msg_body_final=urlencode($msg);
								 ...
								 $URL="http://roundsms.com/api/sendhttp.php?authkey=OTFmM2Y1ZDJkNDd&mobiles=".$phone."&message=".$msg_body_final."&sender=ZOPRNT&type=1&route=3";
		                        $ch = curl_init();
								curl_setopt($ch, CURLOPT_URL, $URL);
								curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
								curl_exec($ch);
...
&noscript=1"/>
		...
		<script>
			function change_date()
			{
				...
				to_date_a=from_date;
				$("#to_date").val(to_date_a);
				to_date=$("#to_date").val();
				 
				jQuery.ajax({
						type:"POST",
						url:"ajax.php?f=changes_date",
						datatype:"text",
	...
	function month_location(id)
	{
		...
		jQuery.ajax({
						type:"POST",
						url:"ajax.php?f=months_location",
						datatype:"text",
...
<body class="res layout-subpage">
  ...
  <div class="container padding-bottom-top-120 text-uppercase text-center" style="padding: 41px 0;">
    <div class="main-title" style="padding-top: 4%;padding-bottom: 2%;">
      <h1 style="color:#fff;font-family:ab2;">Your product is here!</h1>
				...
				<div id="content" class="col-md-12 col-sm-12">
					...
					<?php
						...
						if(count($id)>=1)
						{
							...
							$test_qry="select * from product_details where (sub_category='$sub_id' and vendor_city='$city_id') and $ven_value and monthly > 0";	
						}
						else
						{
							$test_qry="select * from product_details where (sub_category='$sub_id' and vendor_city='$city_id') and monthly > 0";
						}
...
<h3 style="text-align:center;color:#004066;font-size:15px;font-family:ab2;font-size:20px;    font-weight: bold;"><?php echo $pro_name;?></h3>
									...
									<center><img src="admin/<?php echo $image;?>" alt="Blank_Image"></center>
									</div>
    ...
    margin-top: 3%;">Location</h3>
														 ...
														 <div class="col-md-6 col-sm-12">
														  <div class="form-group ">
															<label for="input-payment-firstname" class="control-label" style="font-family: ab2;color: #606062;">To Date</label>
																<input type="text" class="form-control" id="to_date" placeholder="To Date" value="<?php echo $_SESSION['to_date'];?>"  name="to_date" disabled="disabled">
														  </div>
														  </div>
														</form>
														</fieldset>-->
											...
											<div class="col-md-12 col-sm-12 col-xs-12" style="font-family: ab2;" id="product_details">
													<?php 
										if($count>=1)
										{
													echo '
												...
												<div class="container">
															<div class="col-md-12 col-sm-12 col-xs-12">
																	<div class="col-md-6">
																		...
																		<h5 style="font-size: 20px;font-weight: bold;"><i class="fa fa-inr"></i> '.$price.' / month</h5>
																	</div>
										...
										<div class="row">
											...
											<div class="col-md-12" style="padding-top: 1%;font-family: ab2;">
												...
												$diff=$qty-$booked_count;
														...
														if($diff>=1)
														{
															echo '<option value="'.$vendor_id.'">'.$ven_name.' ('.$ven_loc.')- Rs. '.$ven_total_price.' (Available)</option>';
														}
														else
														{
															echo '<option value="'.$vendor_id.'" disabled>'.$ven_name.' ('.$ven_loc.')- Rs. '.$ven_total_price.' (Not Available)</option>';
														}													
